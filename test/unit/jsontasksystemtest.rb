...
class JiraTaskSystemTest < Test::Unit::TestCase
  ...
  Test::Unit.at_start do
	...
	def test_jira_rainy_day_scenario_host_exits_page_not_found
		settings = { :name => 'json', :query_string => 'http://www.praqma.com/#{task_id}', :usr => 'usr', :pw => 'pw' }
		collection = Model::PACTaskCollection.new
		task = Model::PACTask.new 'Jira1'
		task.applies_to = 'json'
		collection.add(task)
		jira = Task::JsonTaskSystem.new(settings).apply(collection)
		assert_false(jira)
	end
	...
	def test_jira_rainy_day_scenario_host_not_exists
		settings = { :name => 'json', :query_string => 'http://we.are.not.doughtnuts.fix/#{task_id}', :usr => 'usr', :pw => 'pw' }
		collection = Model::PACTaskCollection.new
		task = Model::PACTask.new 'Jira1'
		task.applies_to = 'json'
		collection.add(task)
	...
	def test_parsing_of_proper_json
		...
		task.applies_to = 'json'
		collection.add(task)
		rest = Task::JsonTaskSystem.new(settings).apply(collection)

...
then
	...
	docker run --name $DB -d -e 'DB_USER=jiradb' -e 'DB_PASS=jellyfish' -e 'DB_NAME=jiradb' praqma/pac_test_postgres-for-jira:v1
	docker run -d --name $JIRA -e "JIRA_DATABASE_URL=postgresql://jiradb@postgres/jiradb" -e "JIRA_DB_PASSWORD=jellyfish" --link $DB:postgres -p $HOSTPORT:8080 praqma/pac_test_jira:v1
